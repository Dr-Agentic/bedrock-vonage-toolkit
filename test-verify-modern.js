...
if (args.length < 1) {
  console.error('Usage: node test-verify-modern.js <command> [params...]');
...
async function requestVerification() {
  try {
    ...
    const result = await verify.start({
      number: PHONE_NUMBER,
      brand: BRAND_NAME,
      workflow_id: 6 // Use SMS and then voice call
    });
    ...
    if (result.requestId) {
      ...
      console.log(`node test-verify-modern.js check ${result.requestId} <code>`);
      
      // Save request ID to a file for later use
      require('fs').writeFileSync('last-request-id.txt', result.requestId);
        ...
        if (args.length < 3) {
          ...
          if (!requestId) {
            try {
              ...
              console.error('Usage: node test-verify-modern.js check <requestId> <code>');
              process.exit(1);
            }
          }
        ...
        if (args.length < 2) {
          ...
          if (!requestId) {
            try {
              ...
              console.error('Error reading request ID from file.');
              console.error('Usage: node test-verify-modern.js cancel <requestId>');
              process.exit(1);
            }
          }
