...
if (args.length < 1) {
  console.error('Usage: node test-local-serverless.js <command> [params...]');
...
async function testRequestVerification() {
  try {
    ...
    if (response.data && response.data.requestId) {
      ...
      console.log(`node test-local-serverless.js check ${response.data.requestId} <code>`);
        ...
        if (args.length < 3) {
          ...
          if (!requestId) {
            try {
              ...
              console.error('Usage: node test-local-serverless.js check <requestId> <code>');
              process.exit(1);
            }
          }
        ...
        if (args.length < 2) {
          ...
          if (!requestId) {
            try {
              ...
              console.error('Error reading request ID from file.');
              console.error('Usage: node test-local-serverless.js cancel <requestId>');
              process.exit(1);
            }
          }
